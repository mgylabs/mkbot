...
def requests_API(method, link, datadict=None):
    method = method.upper()
    if method == "POST":
        x = requests.post(base_url + link, data=datadict, headers=req_headers)
    elif method == "PUT":
        x = requests.put(base_url + link, data=datadict, headers=req_headers)
    elif method == "GET":
...
def pre_release():
    ...
    for d in mr_data:
        requests_API(
            "PUT",
            "/projects/{}/merge_requests/{}".format(project_id, d["iid"]),
            {"add_labels": "workflow::verification"},
        )
...
def stable_release():
    ...
    for k, v in data_dict.items():
        ls = requests_API(
            "GET", "/projects/{}/repository/commits/{}/refs".format(project_id, v)
        ).json()
        for x in ls:
            if (x["type"] == "tag") and (x["name"] == tag_name):
                requests_API(
                    "PUT",
                    "/projects/{}/merge_requests/{}".format(project_id, k),
                    {"add_labels": "workflow::production"},
                )
                break

    requests_API(
        "PUT",
        "/projects/{}/badges/{}".format(project_id, 79762),
        {
            "image_url": "https://img.shields.io/badge/Stable Release-{}-00aced?logo=Azure+Pipelines&logoColor=white".format(
                tag_name.replace("-", "--")
            )
        },
    )
