...
async def poll(ctx: commands.Context, question, *candidates):
    ...
    if len(candidates) <= 1:
        await channel.send(embed=MsgFormatter.get(ctx, 'You need more than two candidates!'))
        return
    elif len(candidates) > 10:
        await channel.send(embed=MsgFormatter.get(ctx, 'You cannot make more than 10 candidates!'))
        return
    else:
