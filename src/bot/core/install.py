...
async def install(ctx: commands.Context, ext_id: str, option=None):
    ...
    if ext_data == None:
        await ctx.send(embed=MsgFormatter.get(ctx, f"{ext_id} 확장을 찾을 수 없습니다."))
        return
    ...
    if ext_data.get("auth", False):
        if len(CONFIG.gitlabToken) == 0:
            ...
            req_headers = {"Authorization": "Bearer " + CONFIG.gitlabToken}
        else:
            req_headers = {"Authorization": "Bearer " + CONFIG.gitlabToken}
    ...
    if (cur_commit == None) or ((option == "-U") and (cur_commit != last_commit)):
        ...
        download_file_name = os.getenv("TEMP") + f"\\mkbot-update\\{name}.zip"
        ...
        await ctx.send(
            embed=MsgFormatter.get(
                ctx, f"{ext_id} 확장 설치가 완료되었습니다.", "확장을 적용하려면 MK Bot을(를) 다시 시작하세요."
            )
        )
    elif option == "-U":
        await ctx.send(
            embed=MsgFormatter.get(
                ctx, "사용 가능한 업데이트가 없습니다.", f"**{ext_id}**\n{last_commit[:8]}"
            )
        )
    else:
        await ctx.send(
            embed=MsgFormatter.get(
                ctx, "이미 설치된 확장입니다.", f"`{ctx.message.content} -U` 을(를) 시도해 보세요."
            )
        )
