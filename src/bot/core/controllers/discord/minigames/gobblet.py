...
class Player:
    ...
    def content(self) -> str:
        return __("It is now %(kind)s %(member)s's turn.") % {
            "kind": self.kind,
...
class PlayerPromptButton(discord.ui.Button["PlayerPrompt"]):
    def __init__(
        self, style: discord.ButtonStyle, number: int, disabled: bool, row: int
    ) -> None:
...
class PlayerPrompt(discord.ui.View):
    def __init__(self, player: Player, state: BoardState):
        ...
        for x in range(0, 6):
            ...
            self.add_item(
                PlayerPromptButton(player.kind.style, number, disabled, row=y)
            )
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user != self.player.member:
            await interaction.response.send_message(
                __("This is not meant for you"), ephemeral=True
            )
            return False
        return True
...
class Button(discord.ui.Button["Gobblers"]):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if state.kind == player.kind:
            await interaction.response.send_message(
                __("You already have this piece"), ephemeral=True
            )
            return

        if player.available_strength <= state.strength:
            await interaction.response.send_message(
                __("You do not have the strength necessary to take down this piece"),
                ephemeral=True,
            )
            return
        ...
        await interaction.response.send_message(
            __("Select a piece strength"), view=prompt
        )
        await prompt.wait()
        ...
        if winner is not None:
            if winner is not BoardKind.Empty:
                ...
                content = __("%(winning_player)s won!") % {
...
class Gobblers(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user not in (self.players[0].member, self.players[1].member):
            await interaction.response.send_message(
                __("This is a game between two other people, sorry."), ephemeral=True
            )
            return False

        if interaction.user != self.current_player.member:
            await interaction.response.send_message(
                __("It's not your turn"), ephemeral=True
            )
            return False
    ...
    def get_winner(self) -> Optional[BoardKind]:
        ...
        for line in range(3):
            value = (
                self.board[0][line].kind.value
                + self.board[1][line].kind.value
                + self.board[2][line].kind.value
            )
            if value == 3:
                return BoardKind.O
            elif value == -3:
        ...
        # Check diagonals
        diag = (
            self.board[0][2].kind.value
            + self.board[1][1].kind.value
            + self.board[2][0].kind.value
        )
        if diag == 3:
            return BoardKind.O
        elif diag == -3:
        ...
        diag = (
            self.board[0][0].kind.value
            + self.board[1][1].kind.value
            + self.board[2][2].kind.value
        )
        if diag == 3:
            return BoardKind.O
        elif diag == -3:
...
class Prompt(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user != self.second:
            await interaction.response.send_message(
                __("This prompt is not meant for you"), ephemeral=True
            )
            return False
        return True
    ...
    async def decline(
        ...
        await interaction.response.send_message(__("Challenge declined :("))
        self.stop()
