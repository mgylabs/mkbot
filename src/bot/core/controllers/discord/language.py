    ...
    async def set(
        ...
        await ctx.send(
            embed=MsgFormatter.get(
                ctx,
                "Language",
                __("Successfully set display language of %(member)s to %(language)s")
        ...
        if user := DiscordUser.get_one_or_none(id=member.id):
            await ctx.send(
                embed=MsgFormatter.get(
                    ...
                    __("Display language of %(member)s") % {"member": member.mention},
                ),
                ephemeral=True,
            )
        else:
            await ctx.send(
                embed=MsgFormatter.get(
                    ctx,
                    "Language",
                    f"{__('Display language of %(member)s') % {'member': member.mention}}: English",
                ),
                ephemeral=True,
            )
