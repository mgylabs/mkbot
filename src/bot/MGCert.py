                ...
                if level != Level.ALL_USERS:
                    ...
                    if (len(rusers) != 0):
                        if not (req_user in rusers):
                            ...
                            embed = replyformat.get(ctx, "Permission denied", '사용자 <@{}>은(는) {} 목록에 등록되어 있지 않습니다. 이 시도를 보고합니다.'.format(ctx.author.id, Level.get_description(level)), False)
                            embed.add_field(name='User',value=str(ctx.author))
                            embed.add_field(name='Command tried', value='{} ({})'.format(ctx.command.name, Level.get_description(level)))
                            await ctx.send(embed=embed)
                            logger.critical('{} tried to command that needs "{}" permission.'.format(req_user, Level.get_description(level)))
