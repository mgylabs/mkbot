...
class TokenEater:
    ...
    def __suitedocstring(self, ttype, tstring, lineno):
        ...
        if ttype == tokenize.STRING and is_literal_string(tstring):
            result = inspect.cleandoc(safe_eval(tstring))
            self.__addentry(result, lineno, isdocstring=1)

            fisrt_line = result.split("\n", 1)[0]
            if fisrt_line != result:
                self.__addentry(fisrt_line, lineno, isdocstring=1)
